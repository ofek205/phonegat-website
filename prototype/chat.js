...
(function(){
  ...
  function findField(text){
    ...
    for(var i=0;i<DEV_FIELDS.length;i++){
      ...
      for(var j=0;j<kws.length;j++)if(hay.indexOf(normDev(kws[j]))>=0)return DEV_FIELDS[i][0];
    }
    return null;
  }
  ...
  function handle(text){text=(text||'').trim();if(!text)return;lastUser=text;bubble(text,'user');if(flow){handleFlow(text);return;}var it=matchIntent(text);if(it&&ACTION_INTENT[it.id]){respond(it);return;}if(deviceAnswer(text))return;if(contentAnswer(text,8))return;if(it){respond(it);return;}if(contentAnswer(text))return;fallback();}
