...
(function(){
  ...
  var INTENTS=[
    ...
    {id:'celcom',c:1,k:['סלקום','קו','קווים','סים','ניוד','מסלול','חבילה','חבילת','דקות','גלישה'],a:'פון גת הוא משווק מורשה סלקום בקרית גת: פתיחת קו וניוד, החלפת סים ומסלולים, וגם שירות לכל חברות הסלולר.',cta:['tel','wa'],sug:['callback']},
...
botReply(ctx.device+' עם '+iss+', בול בתחום שלנו. '+warr+' המחיר תלוי בדגם, אז בואו נדבר ונגיד לכם בדיוק כמה.\n'+hoursLine(),{cta:contactCta(),waText:wt,sug:[['שאלה אחרת',function(){mainMenu();}]]});}
  ...
  function dirBest(text){
    ...
    var toks=qTokens(text);
    /* **מילה אחת מספיקה כאן, בשונה משכבת התוכן.** "כמה אחסון צריך" נשאר עם טוקן אחד
       אחרי שמילות העצירה יורדות, ולכן הוא נפל לגמרי, למרות שיש מדריך ששמו בדיוק זה.
       שם עמוד הוא שלוש עד שש מילים ולא פסקה, ולכן מילה נדירה אחת בו היא אות חזקה.
       הסינון נשאר בניקוד, שנשען על idf, ולא בספירת מילים. */
...
var txt=fails>=2?'עדיין לא הצלחתי להבין. הכי טוב לדבר ישירות עם ברוך וסיגל, או להשאיר פרטים ונחזור אליכם:':'לא בטוח שהבנתי. אפשר לנסות שוב במילים אחרות, או לבחור נושא:';botReply(txt,{cta:contactCta(),sug:['repair','buy','callback']});}
  ...
  function handle(text,waited){
    ...
    /* ---- בקשה מול תיאור: מי מנצח, הזרימה או המדריך ----
     *
     * **התלונה שנמדדה:** שש שאלות תקלה שונות קיבלו את אותה שאלה חזרה, "איזה מכשיר צריך
     * טיפול?", במקום את המדריך שנכתב בדיוק בשבילן:
     *   "הטלפון לא נדלק"  ·  "נפל לי הטלפון למים"  ·  "הסוללה נגמרת מהר"
     *   "כמה זמן לוקח להחליף מסך"  ·  "המסך שלי שבור"  ·  "אתם מתקנים אייפון"
     * לקוח שמתאר תסמין רוצה לדעת מה קורה, ובמקום זה הוא נשאל שאלה. זה מה שמרגיש כמו
     * "הבוט רק מעביר אותי לשלוח הודעה".
     *
     * **הסיבה, וזו רגרסיה שלי מ-17.8.2026:** כשדירוג הכוונות עבר לספירת התאמות, repair
     * התחיל לנצח כמעט כל תיאור תסמין, כי 17 מילות המפתח שלו הן בדיוק אוצר המילים של
     * תקלות. וכוונות פעולה נענות לפני שכבת התוכן, ולכן המדריכים הפכו לבלתי נגישים.
     *
     * **ההבחנה שפותרת:** בקשה מפורשת מול תיאור.
     *   "אני רוצה לתקן מסך"   בקשה  → זרימה, כי הלקוח מבקש שירות
     *   "המסך שלי שבור"       תיאור → קודם מדריך, ואם אין, זרימה
     * לכן פועל פעולה מפורש שולח לזרימה מיד, וכל השאר נותן לתוכן הזדמנות ראשונה בסף 8.
     * הזרימה לא אבדה: contentAnswer מחזיר false כשאין התאמה, והשורה הבאה פותחת אותה. */
    var EXPLICIT=/תיקון|לתקן|מתקנ|רוצה לקנות|לקנות|לרכוש|קונה|תחזרו|תתקשרו|שיחה חוזרת|הצעת מחיר|אשמח להצעה/;
    /* **שאלה שמחפשת עצה אינה בקשת שירות, גם אם יש בה פועל פעולה.**
       "כדאי לקנות את הדגם החדש או הקודם" פתח את זרימת הקנייה בגלל המילה "לקנות", בזמן
       שיש לנו מדריך ששמו בדיוק זה. מילת שאלה בפתח המשפט גוברת על הפועל. */
    var ASKS=/^\s*(כדאי|עדיף|האם|מה |מהו|איזה|איזו|איך|כמה|מתי|איפה|למה|האם)/;
    if(it&&ACTION_INTENT[it.id]){
      if(EXPLICIT.test(text)&&!ASKS.test(text)){respond(it);return;}
      if(contentAnswer(text,8))return;
      respond(it);return;
    }
