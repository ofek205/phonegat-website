...
botReply(ctx.device+' עם '+iss+', בול בתחום שלנו. '+warr+' המחיר תלוי בדגם, אז בואו נדבר ונגיד לכם בדיוק כמה.\n'+hoursLine(),{cta:contactCta(),waText:wt,sug:[['בקשת שיחה חוזרת',function(){startLead(ctx.topic);}],['שאלה אחרת',function(){mainMenu();}]]});}
  ...
  var PROBLEM=/שבור|שבורה|סדוק|נסדק|לא עובד|לא נדלק|לא נטען|לא מגיב|לא מזהה|נגמרת|מתרוקנת|נפל|נשפך|רטוב|תקוע|נתקע|כבה|מקרטע|איטי|מתחמם|נדפק|התקלקל/;
  function pageSlug(){
  ...
  function unknownModelReply(text){
    ...
    botReply('את הדגם הזה אני לא מוצא ברשימה שלנו. אפשר לראות מה יש אצלנו, או לשאול את ברוך אם אפשר להשיג אותו במיוחד.',
      {sug:[['רשימת המכשירים',function(){startBuy();}],['לשאול את ברוך',function(){startLead('בקשה לדגם');}]]});
  }
