...
function structure() {
  return [
    /* ראשון ברשימה, כלומר הימני ביותר בהדר והעליון ביותר במגירה. אופק ביקש את זה
       ב-16.8.2026, אחרי שהלחצן ישב באמצע ולא נמצא. המיקום הוא מה שעושה כאן את
       העבודה, יחד עם התווית, ולא רקע צבעוני.
       התווית נבחרה על ידי אופק. "בין הדגמים" הוא מה שמפריד אותה מ"מדריכי השוואה"
       שיושב בתפריט המכשירים והוא 19 מאמרים לקריאה, ו"כלי" אומר שמפעילים אותו. */
    { type: 'link', href: '/phones/compare/', label: 'כלי השוואה בין הדגמים', cls: 'ntool' },
    {
      type: 'drop', id: 'nd-devices', label: 'מכשירים',
      cols: deviceItems(),
      all: [
        /* שתי התוויות האמצעיות היו "השוואות בין דגמים" ו"השוואת מכשירים", שתי מילים כמעט
           זהות לשני דברים שונים לגמרי: 19 מאמרים כתובים מול כלי אינטראקטיבי. ההערה שהייתה
           כאן קודם כבר הסבירה את ההבדל, וזה בדיוק הסימן שהתוויות לא עשו זאת בעצמן.
...
function CSS(t) {
  ...
  /* כלי ההשוואה: אותו פריט כמו כל השאר, ורק המשקל מבדיל. עבר דרך מסגרת ודרך רקע מלא,
     ...
     שאומרת מה מקבלים, ולא הצבע. 800 מול 600 של האחים, על אותו צבע טקסט (16.36:1),
     ולכן אין כאן שאלת ניגודיות בכלל וגם אין תחרות עם כפתור ה-WhatsApp הירוק שבהדר. */
  'nav.main a.ntool{font-weight:800}',
