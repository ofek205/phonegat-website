...
var CATS = {
  watches: {
    ...
    title: 'השוואת שעונים חכמים: אפל וסמסונג | פון גת',
    desc: 'כלי להשוואה בין שעונים חכמים של אפל וסמסונג, עם המפרט מאתרי היצרנים. רק השדות שבהם הם באמת שונים.',
    h1: 'השוואת שעונים חכמים',
    asub: 'שעוני Apple Watch ו-Galaxy Watch, והמפרט של כל אחד מאתר היצרן. לפני הכול בדקו את השורה "עובד עם": Apple Watch עובד רק עם אייפון, ושעוני Galaxy רק עם אנדרואיד.',
...
function toolMain(openTag, index, order, pairCount) {
  ...
  ['א׳', 'ב׳'].map(function (s, i) {
    return '        <li class="dslot empty"><button type="button" class="dopen" data-slot="' + i +
      ...
      '<span class="lbl">' + (CAT ? CAT.slot : 'מכשיר') + ' ' + s + '</span><span class="nm">בחרו דגם</span></span></button></li>';
  }).join('\n') + '\n' +
  '        <li><button type="button" class="dadd" data-add="1">' + (CAT ? CAT.third : 'הוסיפו מכשיר שלישי') + '</button></li>\n' +
  '      </ul>\n' +
...
'    </div>\n  </div>\n</section>\n\n' +
  ...
  '<section class="cta" aria-labelledby="cta-h">\n  <div class="wrap">\n' +
  /* בכלי קטגוריה איננו יודעים אילו דגמים בחנות. עד 24.9.2026 גם כלי השעונים אמר כאן
     "המכשירים אצלנו בחנות", כי הטקסט הגיע מכלי הטלפונים. שאלה, לא הבטחה. */
  (CAT
...
function buildTool() {
  ...
  if (CAT) {
    ...
    if (!db._means) { console.error('✗ ' + CAT.key + ': אין _means ב-' + CAT.file); process.exit(1); }
