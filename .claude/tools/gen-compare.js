...
var APP_CSS = [
  ...
  '.dmenu .dmlist .blogo-google{block-size:15px}',
  '.dmenu .dmlist .blogo-oneplus{block-size:15px}',
  '.dmenu .dmlist .blogo-nothing{block-size:10px}',
  /* ראש הטבלה. שלושת הראשונים מוגדרים בגוש ה-CSS של הכלי, ואלה ממשיכים אותם. */
  '.cmp-grid thead .blogo-google{height:13px}',
  '.cmp-grid thead .blogo-oneplus{height:13px}',
  '.cmp-grid thead .blogo-nothing{height:9px}',
  ...
  /* דגם שאיננו מוכרים: מסומן כבר בבחירה ולא רק בתוצאה. קו מקווקו ולא צבע, כי הוא לא
     פחות טוב ולא לא-זמין, הוא פשוט לא שלנו. הטקסט נשאר קריא ואינו מעומעם. */
  '.dchip.dref{border-style:dashed}',
  '.dchip .rf{font-size:.82rem;font-weight:500;color:var(--ink-soft);margin-inline-start:.45rem;white-space:nowrap}',
  /* לבן מלא ולא 85 אחוז: על טורקיז ההפרש בין השניים הוא בין 4.89:1 לבין כ-4.3, כלומר
     בין לעבור את הסף לבין ליפול מתחתיו, ואין שום דבר שנקנה בשקיפות הזאת. */
  '.dchip.sc0 .rf,.dchip.sc1 .rf{color:#fff}',
  '.dchip.sc2 .rf{color:#3c2e12}',
  '.dmenu .dmlist .cnt.rf{font-family:var(--font);font-size:.9rem}',
  /* ================= גבולות פקדים, המשך
...
var BRAND_LOGO = {
  Apple: ['apple', 15, 18], Samsung: ['samsung', 34, 12], Xiaomi: ['xiaomi', 16, 16],
  ...
  Google: ['google', 16, 16], Nothing: ['nothing', 62, 11], OnePlus: ['oneplus', 16, 16]
};
...
var BRAND_LOGO_CELL = {
  Apple: ['apple', 12, 14], Samsung: ['samsung', 29, 10], Xiaomi: ['xiaomi', 13, 13],
  Google: ['google', 13, 13], Nothing: ['nothing', 50, 9], OnePlus: ['oneplus', 13, 13]
};
 ...
 */
var TFMT = {
  weight:          { fmt: 'int', unit: ' גרם', gap: true, low: true, lowMore: 'קל יותר',
  ...
  battery_hours:   { fmt: 'int', unit: ' שעות', group: 'batt', who: 'אם אתם מחוץ לבית כל היום' },
  battery_mah:     { fmt: 'int', unit: ' mAh', group: 'batt', who: 'אם אתם מחוץ לבית כל היום' },
...
var TDEF = T.DELTAS.map(function (d) {
  ...
  return { key: d.key, label: d.label, min: d.min, more: d.more, fmt: f.fmt, unit: f.unit,
           each: !!f.each, gap: !!f.gap, low: !!f.low, lowMore: f.lowMore || d.more,
           group: f.group || d.key, zero: f.zero || null, who: f.who };
});
  ...
  brands.map(function (b) {
    ...
    return '        <div class="dgrp">\n          <p class="dbrand">' + (L ? '<img class="blogo blogo-' + L[0] + '" src="/logos/' + L[0] +
      ...
      b.items.map(function (d) {
        ...
        /* דגם ייחוס מסומן כבר כאן, לפני הבחירה. הגילוי הנאות הופיע רק אחרי שההשוואה נבנתה,
           כלומר הלקוח היה מגלה שאיננו מוכרים את הדגם רק אחרי שכבר השקיע בו בחירה. */
        var ref = d.status === 'reference';
        return '            <li><button type="button" class="dchip' + (ref ? ' dref' : '') + '" data-slug="' + esc(d.slug) +
          '" data-q="' + esc(d.name + ' ' + (d.name_he || '') + ' ' + d.brand + ' ' + (BRAND_HE[d.brand] || '')) +
          '" aria-pressed="false">' + ltr(d.name) +
          (ref ? '<span class="rf">לא אצלנו</span>' : '') + '</button></li>';
      }).join('\n') + '\n          </ul>\n        </div>';
  }).join('\n') + '\n      </div>\n' +
