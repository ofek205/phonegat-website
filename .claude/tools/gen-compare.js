...
var WATCHES = process.argv.indexOf('--watches') >= 0;
var db = JSON.parse(fs.readFileSync(path.join(PROTO, WATCHES ? 'watches.json' : 'devices.json'), 'utf8'));
...
var only = WATCHES ? null : process.argv[2];
...
var APP_CSS = [
  ...
  /* השנה בראש הטור. מעומעמת וקטנה, כי היא הקשר ולא הכותרת, ו-nowrap כדי שלא תישבר
     מהשם שלפניה. font-variant-numeric כדי שהספרות יהיו באותו רוחב בכל הטורים. */
  '.cmp-grid thead .dhyear{font-weight:500;font-size:.82em;color:var(--ink-soft);white-space:nowrap;font-variant-numeric:tabular-nums;margin-inline-start:.3rem}',
...
function toolMain(openTag, index, order, pairCount) {
  ...
  if (!WATCHES) live.forEach(function (d) { TRAITS[d.slug] = traitsOf(d); });
  var waPick = wa(WATCHES ? 'היי, אני מתלבט בין כמה שעונים חכמים ואשמח לעזרה בבחירה'
                          : 'היי, אני מתלבט בין כמה דגמים ואשמח לעזרה בבחירה');
  ...
  (WATCHES
    ? '    <h1 id="h1">השוואת שעונים חכמים</h1>\n' +
      '    <p class="asub">שעוני Apple Watch ו-Galaxy Watch, והמפרט של כל אחד מאתר היצרן. לפני הכול בדקו את השורה "עובד עם": Apple Watch עובד רק עם אייפון, ושעוני Galaxy רק עם אנדרואיד.</p>\n' +
      '    <p class="ahelp">מעדיפים שנעבור על זה יחד? <a href="' + waPick + '">כתבו לנו ב-WhatsApp</a>. רוצים להשוות טלפונים? <a href="/phones/compare/">לכלי הטלפונים</a>.</p>\n'
    : '    <h1 id="h1">השוואת מכשירים</h1>\n' +
      ...
      '    <p class="ahelp">מעדיפים שנעבור על זה יחד? <a href="' + waPick + '">כתבו לנו ב-WhatsApp</a>. רוצים להשוות שעונים חכמים? <a href="/watches/compare/">לכלי השעונים</a>.</p>\n') +
  '  </div>\n</section>\n\n' +
  ...
  (WATCHES
    ? '      <p class="dempty">כאן תופיע טבלת ההבדלים. הכלי צריך JavaScript כדי לעבוד.</p>\n'
...
'    </div>\n  </div>\n</section>\n\n' +
  ...
  (WATCHES
    ? '      <p>רשימת השדות השונים בכל זוג מחושבת מראש, מתוך המפרט שפרסם היצרן. אפל וסמסונג מודדות סוללה בדרכים שונות, ולכן השורה הזאת מראה את מה שכל יצרן אמר ולא מספר אחד שאפשר להשוות.</p>\n'
...
function buildTool() {
  ...
  var TOOL_PATH = WATCHES ? 'watches/compare/' : 'phones/compare/';
  var toolUrl = PROD + TOOL_PATH;
  var title = WATCHES ? 'השוואת שעונים חכמים: אפל וסמסונג | פון גת' : 'השוואת מכשירים: בחרו שני דגמים | פון גת';
  var desc = WATCHES
    ? 'כלי להשוואה בין שעונים חכמים של אפל וסמסונג, עם המפרט מאתרי היצרנים. רק השדות שבהם הם באמת שונים.'
  ...
  var toolCrumbs = '<script type="application/ld+json">\n' + JSON.stringify({
    '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: WATCHES ? 'השוואת שעונים' : 'השוואת מכשירים', item: toolUrl }
    ]
  }) + '\n</script>';
  h = h.replace(/<script type="application\/ld\+json">\s*\{"@context":"https:\/\/schema\.org","@type":"Product"[\s\S]*?<\/script>\s*/, '');
  ...
  /* מצב שעונים: קוד הדפדפן זהה, חוץ מחמישה מקומות שמניחים טלפון. כל החלפה נבדקת שקרתה בדיוק
     פעם אחת, כי החלפה שלא נתפסה משאירה בכלי השעונים את ההסברים של טלפון, בלי שום סימן. */
  if (WATCHES) {
    var once = function (from, to, what) {
      var c = h.split(from).length - 1;
      if (c !== 1) { console.error('✗ watches: ' + what + ', ציפיתי למופע אחד ונמצאו ' + c); process.exit(1); }
      h = h.replace(from, to);
    };
    once('fetch("/devices-public.json"', 'fetch("/watches-public.json"', 'כתובת קובץ הנתונים');
    once('var NUMOK={screen_size:1,weight:1,brightness:1,security_updates:1};',
         'var NUMOK=' + JSON.stringify(db._numok || {}) + ';', 'NUMOK');
    var mi = h.indexOf('var MEANS={'), me = mi < 0 ? -1 : h.indexOf('};', mi);
    if (mi < 0 || me < 0) { console.error('✗ watches: לא נמצא MEANS'); process.exit(1); }
    if (!db._means) { console.error('✗ watches: אין _means ב-watches.json'); process.exit(1); }
    h = h.slice(0, mi) + 'var MEANS=' + JSON.stringify(db._means).replace(/</g, '\\u003c') + h.slice(me + 1);
    once('לא הצלחנו לחשב את ההשוואה הזאת. <a href="/compare/">ההשוואות המוכנות</a> זמינות תמיד.',
         'לא הצלחנו לחשב את ההשוואה הזאת. נסו לבחור שוב.', 'הודעת שגיאה 1');
    once('לא ניתן לטעון את נתוני המכשירים. <a href="/compare/">ההשוואות המוכנות</a> עובדות בלי הכלי.',
         'לא ניתן לטעון את נתוני השעונים. נסו לרענן את העמוד.', 'הודעת שגיאה 2');
    /* הקובץ הציבורי: רק מה שהכלי קורא, כמו devices-public.json */
    var pub = { _: 'נגזר מ-watches.json על ידי gen-compare.js --watches. אל תערוך ביד.',
    ...
    fs.writeFileSync(path.join(PROTO, 'watches-public.json'), JSON.stringify(pub) + '\n');
  }
  ...
  try {
    ...
    if (!svc.existing.filter(function (x) { return x.url === '/' + TOOL_PATH; }).length) {
      svc.existing.push({ url: '/' + TOOL_PATH, name: WATCHES ? 'השוואת שעונים' : 'כלי ההשוואה', status: 'review' });
