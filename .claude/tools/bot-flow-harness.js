...
/* טקסט הבועה האחרונה של הבוט. bubble() בונה div.pg-b ובתוכו span עם textContent,
   ולכן הטקסט יושב בילד הראשון. נדרש כדי למדוד **מה** הבוט ענה ולא רק לאן השאלה נחתה:
   אירוע יכול להיות "נכון" בזמן שהתשובה עצמה אינה אומרת ללקוח שום דבר. */
function lastBot() {
  var out = null;
  (function walk(n) {
    if (!n || !n.children) return;
    n.children.forEach(function (c) {
      if (String(c.className || '').indexOf('pg-bot') >= 0) {
        var kid = (c.children || [])[0];
        if (kid && kid.textContent) out = String(kid.textContent);
      }
      walk(c);
    });
  })(els.pgMsgs);
...
var CASES = [
  ...
  /* תיאור תסמין: מקבל את המדריך, ולא שאלה חזרה. ראו התלונה שנמדדה ב-handle. */
  ['המסך שלי שבור', 'chat_content_hit'],
  ...
  /* **הוחזר למדריך ב-17.8.2026, אחרי שהחלפתי אותו לזרימה באותו יום וטעיתי.**
     כתבתי אז שזה "שיפור ולא נסיגה", וזה היה לא נכון. הבעלים דיווח שהוא לא מצליח להגיע
     למידע ושהבוט רק מעביר אותו לשלוח הודעה, ומדידה על 40 שאלות הראתה שש שאלות תקלה
     שכולן קיבלו את אותה שאלה חזרה, "איזה מכשיר צריך טיפול?", במקום את המדריך.
     תיאור של תסמין הוא שאלה, לא בקשת שירות. ראו EXPLICIT ב-handle. */
  ['הטלפון נפל למים מה עושים', 'chat_content_hit'],
  /* ובקשה מפורשת כן נשארת זרימה, וזה מה שמפריד */
  ['אני רוצה לתקן מסך', 'chat_flow:repair'],
  ['כמה זמן לוקח להחליף מסך', 'chat_content_hit'],
  ...
  /* היה כאן "יש לכם אייפון 12", ואייפון 12 נוסף למאגר ב-17.8.2026 כמכשיר ייחוס. מרגע
     שהבוט מכיר דגם הוא אינו נופל יותר ל-unmatched, ולכן רישום הביקוש נבדק עכשיו על דגם
     שבאמת אינו במאגר. אייפון 8 אינו במאגר ואינו מתוכנן להיכנס אליו. */
  ['יש לכם אייפון 8', 'chat_device_unmatched'],
  /* ושאלת "יש לכם" על מכשיר ייחוס. מה שנבדק כאן הוא שהיא פותחת זרימת קנייה, כי זו
     ההתנהגות בפועל: הסירוב מגיע בשלב הדגם, אחרי שהלקוח נוקב בשם. buyModel מטפל בזה
     ומחזיר chat_ref_declined, וזה נבדק בנפרד למטה על "גוגל פיקסל 10".

     ⚠️ זה אינו אידיאלי מבחינת המוצר: מי ששואל "יש לכם אייפון 12" מקבל זרימת קנייה
     לדגם שאיננו מוכרים, ולומד שאיננו מוכרים אותו רק צעד אחר כך. התיקון הוא לזהות את
     הדגם כבר בהודעה הפותחת של כוונת הקנייה, וזה שינוי ב-chat.js המשותף. נרשם ולא בוצע. */
  ['יש לכם אייפון 12', 'chat_flow:buy'],
...
(function () {
  ...
  /* מה שנבדק כאן הוא שהקורא יכול להגיע לעמוד, ולא באיזו תווית. מאז 17.8.2026 תקלת המים
     נענית מהמדריך עצמו, ולכן הקישור הוא "לעמוד המלא" של תשובת התוכן ולא צ'יפ "לעמוד: ".
     שתי הצורות מקבילות, והכלל היה נכשל על שיפור. */
  var P2 = boot(0, '/');
  P2.ask('הטלפון נפל למים מה עושים');
  var reach2 = P2.lastSug().filter(function (l) { return /^לעמוד/.test(l); });
  rule('אפשר להגיע מכאן לעמוד שמסביר את תקלת המים', reach2.length >= 1, P2.lastSug().join(' | '));
...
(function () {
  ...
  var cases = [
    ...
    /* מה שהכלל הזה שומר עליו הוא שתלונה **אינה** הופכת לתשובת מפרט על הדגם שבעמוד.
       מאז 17.8.2026 היא מקבלת את המדריך ולא את זרימת התיקון, וזה שיפור; ההגנה נשמרת,
       כי הערך שאסור לראות כאן הוא chat_device_spec. */
    ['תלונה אינה תשובת מפרט', A56, ['המסך שלי שבור'], 'chat_content_hit'],
