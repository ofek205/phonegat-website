...
function complete() { return onScreen().every(function (x) { return !!dev(x); }); }
...
function renderCards() {
  ...
  var topSec = document.querySelector(".cv-top"); if (topSec) topSec.classList.toggle("has-pair", complete());
  renderSug();
}
...
document.addEventListener("click", function (e) {
  ...
  if (chip && pickFor !== null && DB) {
    var i2 = pickFor, s = chip.getAttribute("data-slug"), was = complete(), at = onScreen().indexOf(s);
    ...
    if (sel[i2] && nxt >= 0) { tellSide(nxt); openPick(nxt, true); }
    else if (sel[i2]) closePick(false);
    else openPick(i2, false);
    render();
    if (!was && complete()) toResults();
    return;
  }
  ...
  if (t.closest("#cvdrop")) {
    ...
    render(); if (addBtn) addBtn.focus(); return;
  }
  ...
  if (sg) {
    ...
    if (dev(pr[0]) && dev(pr[1])) { sel = [pr[0], pr[1], null]; slots = 2; push("cmp_suggest", { pair: pr.join(",") }); closePick(false); render(); toResults(); }
    return;
  }
...
function render() {
  renderCards(); syncURL();
  var ds = onScreen().map(dev);
  if (!complete()) {
    cur = null; if (bar) bar.hidden = true; stick();
    var have = ds.filter(Boolean).length;
    out.innerHTML = '<p class="dempty">' + (have ? (slots - have === 1 ? "בחרו עוד דגם" : "בחרו עוד שני דגמים") + ", ותראו כאן את ההבדלים ביניהם." : "בחרו שני דגמים, ותראו כאן את ההבדלים ביניהם.") + "</p>";
    return;
  }
  ...
  if (missingPair) { cur = null; if (bar) bar.hidden = true; out.innerHTML = '<p class="dempty">' + CFG.pairErr + "</p>"; return; }
  ...
  if (bar) {
    ...
    bar.innerHTML = '<div class="wrap cv-bi">' + ds.map(function (d, i) {
      return (i ? '<span class="cv-vs">מול</span>' : "") + '<span class="cv-bn ' + SC[i] + '"><span class="cv-dot" aria-hidden="true"></span>' + ltr(d.name) + "</span>";
      ...
      top = '<h2 class="cv-h2">ההבדלים הגדולים במספרים</h2><ul class="cv-top3' + (gaps.length < 3 ? ' n' + gaps.length : '') + '">' + gaps.map(function (x) {
        ...
        return '<li class="cv-t"><div class="cv-th"><b>' + esc(t.label) + '</b><span class="cv-gap">' + ltrRuns(gapChip(x)) + "</span></div>" +
          '<div class="cv-tv' + (ds.length === 3 ? " n3" : "") + '">' + ds.map(function (d, i) {
            return '<div class="' + SC[i] + '">' + (vs[i] === null ? '<span class="cv-na">לא מפורסם</span>' : bigNum(vs[i], t) + fill(100 * vs[i] / mx)) + '<span class="cv-tn">' + ltr(d.name) + "</span></div>";
          }).join("") + "</div>" +
  ...
  var prio = '<div class="cv-prio"><h2 class="cv-h2">מה חשוב לכם?</h2><p>בחרו תחום, והוא יעלה לראש הרשימה. שום הבדל לא נעלם.</p>' +
    ...
    (anyBar ? '<div class="cv-legend">' + ds.map(function (d, i) { return '<span class="cv-k ' + SC[i] + '"><i aria-hidden="true"></i>' + ltr(d.name) + "</span>"; }).join("") +
  ...
  out.innerHTML = disc + head + top + prio + '<div id="cvlist"></div>' +
    ...
    '<div class="cv-wa"><div class="cv-wt"><b>רוצים שנעבור על זה איתכם?</b><span>ההודעה כבר מוכנה, עם הדגמים ומה שסימנתם. אפשר לערוך אותה לפני השליחה.</span>' +
...
function rowHtml(r, ds) {
  ...
  if (n && n.t && n.max - n.min < n.t.min) small = '<span class="cv-small">הבדל קטן</span>';
  var cell = function (d, i) {
    ...
    return '<div class="cv-v ' + SC[i] + '"><span class="cv-who"><span class="cv-dot" aria-hidden="true"></span>' + ltr(d.name) + "</span>" + v +
...
function sizeHtml(ds) {
  ...
  var fig = function (x, i) { return '<figure class="' + SC[i] + '"><span class="cv-ol" aria-hidden="true" style="width:' + x.w + "px;height:" + x.h + 'px"></span><figcaption>' + ltr(x.h + " × " + x.w) + " מ״מ</figcaption></figure>"; };
  return '<div class="cv-size">' + dd.map(fig).join("") + "<p>בקנה מידה אמיתי, גובה ורוחב בלי עובי. מה שמורגש ביד הוא בעיקר המשקל והעובי.</p></div>";
}
